...
```bash
# 拉取最新代码并更新
git pull && bash start.sh --prebuilt
```
...
# 或直接指定模式
bash start.sh --dev    # 开发模式（本地构建）
bash start.sh --prod   # 生产模式（本地构建）
```
...
| 方式 | 命令 | 构建时间 | 服务器要求 | 适用场景 |
|------|------|----------|------------|----------|
| 预构建镜像 | `./start.sh --prebuilt` | ~10秒 | 1GB+ 内存 | 低配服务器、快速部署 |
| 本地构建 | `./start.sh --prod` | ~15分钟 | 4GB+ 内存 | 自定义代码、内网部署 |
