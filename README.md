...
[![Docker Build](https://github.com/BlkSword/RSS-Post/actions/workflows/docker-build.yml/badge.svg)](https://github.com/BlkSword/RSS-Post/actions/workflows/docker-build.yml)
...
# 待完成任务
1、构建过程中的内存占用，以及nextjs的构建过程过于占用CPU以及内存，对低性能主机极不友好。√（使用预构建镜像解决）

2、当导入OPML时，会产生大量的抓取任务，只可以抓取到一部分资源。但是有很多其实可以抓到，但是都会报错404或者其他超时等问题。并且已经添加的RSS源也没有正确解析（缺少描述一栏）。怀疑问题是队列过载，导致的问题。应当平缓有序的进行。√

3、抓取顺序也应该调整：先是判断可达性（自动发现），然后开始接下来的各项步骤，到最后才是抓取全文，以及AI分析。√

4、应当加入一个功能可以批量重试失败的RSS订阅源。√

5、系统日志依旧不完善，根本看不到RSS抓取的任何日志。即使进行了rss抓取。√

6、系统的搜索功能最多只能最多显示50篇文章。√

7、订阅源中的搜索功能完全无用，不能搜索有关订阅源的任何东西。√

8、应该深入优化AI总结的深度和广度，不应该只停留在一个简报中，应该进行深度分析，并生成一个完整的报告。一开始是总的标题以及总的所有文章的概括。之后是每个大分类的各重点文章以及其各种信息还有各文章摘要。√

9、前端的反应还是太慢了，我觉得要进行相应的优化。Waiting...

10、相关检索算法也应该优化，不能每次都要获取所有内容进行比对才能更新。√

11、系统监控的订阅源状态中的活跃，换成成功获取的数量。√

12、未读文章也只能最多显示50篇文章。√

13、侧边栏的订阅源加载更多不要是跳转，而是继续在下面加载其他的订阅源（不要一下子全展示，分批）。√

14、文章详细页的文章摘要展示不用打字机特效，直接出现即可。√

15、自动AI分析后台要加上，配置AI之前获取的文章，是没法自动生成AI相关的分析的。√

16、报告中心的报告（日报和周报）永远只能生成一份，不能生成更多的。并且去掉AI生成的文字和其图标。√

17、文章详细页，点击生成之后，即使生成成功也不会显示最新状态，还是保持着排队中 (第 x 位)，需要重新刷新页面才能看到信息。√

18、文章详细页中的AI分析中的AI评分，只有总分，深度、质量、实用、新颖这些细化的分数没有显示。√

19、创建分组后，应该紧接的就是一个选择添加订阅源的页面来快速添加相应的订阅源。√

20、应该可以设置抓取文章时间范围（一个月、半年），并且可以定时清理过时的文章。√
...
### 方式一：预构建镜像部署（推荐，低配服务器首选）

**优势**：无需在服务器上构建，几秒钟即可完成部署，适合低性能服务器。
...
# 2. 登录 GitHub Container Registry（镜像公开时可跳过）
echo $GITHUB_TOKEN | docker login ghcr.io -u BlkSword --password-stdin
...
- 创建 `.env` 文件并生成随机安全密钥
- 初始化数据库
- 创建测试账号：`test@example.com` / `password123`
...
**注意**：此方式需要在服务器上构建镜像，会消耗较多 CPU 和内存（建议 4GB+ 内存）。
...
预构建镜像托管在 GitHub Container Registry，每次推送到 main 分支自动构建：

| 镜像 | 用途 | 大小 |
|------|------|------|
| `ghcr.io/blksword/rss-post-app:latest` | 主应用服务 | ~400MB |
| `ghcr.io/blksword/rss-post-worker:latest` | Worker 服务 | ~200MB |
| `ghcr.io/blksword/rss-post-init:latest` | 初始化服务 | ~300MB |

```bash
# 拉取镜像
docker pull ghcr.io/blksword/rss-post-app:latest
docker pull ghcr.io/blksword/rss-post-worker:latest
docker pull ghcr.io/blksword/rss-post-init:latest
```
