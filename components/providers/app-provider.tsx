/**
 * 合并的应用提供者 - 整合主题、语言和 Ant Design 配置
 * 将 ThemeProvider + AntdThemeProvider + LanguageProvider 合并为单一 Provider
 * 减少组件嵌套层级，优化渲染性能
 */
...
const THEME_STORAGE_KEY = 'rss-post-theme';
const LANGUAGE_STORAGE_KEY = 'rss-post-language';
...
export function AppProvider({ children }: AppProviderProps) {
  ...
  useEffect(() => {
    ...
    if (storedLanguage && (storedLanguage === 'zh-CN' || storedLanguage === 'en')) {
  ...
  const antdThemeConfig = useMemo(() => ({
    ...
    token: {
      colorPrimary: '#3b82f6',
      colorBgBase: isDark ? '#0f172a' : '#faf9f7',
      colorTextBase: isDark ? '#e2e8f0' : '#292524',
      colorBorder: isDark ? '#334155' : '#e7e5e4',
      borderRadius: 8,
      wireframe: false,
    },
    components: {
      Card: { colorBgContainer: isDark ? '#1e293b' : '#ffffff' },
      Modal: { colorBgElevated: isDark ? '#1e293b' : '#ffffff' },
      Drawer: { colorBgElevated: isDark ? '#1e293b' : '#ffffff' },
      Menu: {
        colorItemBg: isDark ? '#1e293b' : '#ffffff',
        colorItemBgSelected: isDark ? '#3b82f620' : '#eff6ff',
        colorItemTextSelected: '#3b82f6',
      },
      Input: { colorBgContainer: isDark ? '#1e293b' : '#ffffff' },
      Select: {
      ...
      Tooltip: { colorBgElevated: isDark ? '#475569' : '#1e293b' },
      Popover: { colorBgElevated: isDark ? '#1e293b' : '#ffffff' },
      Tag: { defaultBg: isDark ? '#334155' : '#f5f5f4' },
      Badge: { colorBgContainer: isDark ? '#1e293b' : '#ffffff' },
    },
  }), [isDark]);
  ...
  const contextValue = useMemo<AppContextType>(() => ({
    ...
    setLanguage,
    t,
  }), [mounted, theme, resolvedTheme, setTheme, toggleTheme, language, setLanguage, t]);
...
export function useTheme() {
  const { theme, resolvedTheme, setTheme, toggleTheme } = useApp();
